...
//   * When custom credentials are incomplete or absent, requests fall back to
//     bundled Ollama and never consult LOVABLE_API_KEY.
...
describe("createAiProvider – bundled Ollama fallback", () => {
  it("ignores LOVABLE_API_KEY when CUSTOM_AI_BASE_URL is unset", async () => {
  ...
  it("uses bundled Ollama when CUSTOM_AI_BASE_URL is set but CUSTOM_AI_API_KEY is missing", async () => {
    process.env.CUSTOM_AI_BASE_URL = CUSTOM_BASE;
    process.env.LOVABLE_API_KEY = "lov-key-abc";
    const { calls } = mockChatFetch();
  ...
  it("uses the bundled model when no override is set", async () => {
    process.env.LOVABLE_API_KEY = "lov-key-abc";
    const { calls } = mockChatFetch();
