...
export type ExportAuthorization =
  ...
  | { authorized: false; reason: "site-mismatch"|"snapshot-mismatch"|"module-not-entitled"|"expired"|"already-used"|"invalid-entitlement" };
...
export function authorizeSnapshotExport(entitlement:SnapshotExportEntitlement,request:ExportRequest):ExportAuthorization {
  ...
  let expires:number;try{expires=timestamp(entitlement.expiresAt);timestamp(entitlement.issuedAt);}catch{return {authorized:false,reason:"invalid-entitlement"};}
  ...
  if(request.now.getTime()>=expires)return {authorized:false,reason:"expired"};
