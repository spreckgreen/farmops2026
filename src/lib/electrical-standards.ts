...
export const BUILT_IN_STANDARDS: readonly StandardEntry[] = [
  ...
  },
  {
    ...
    body:
      ...
      "BR-###-##-## — raceway path, the junction box the branch run physically originates from, then the branch-run sequence.\n" +
      "BR-104-02-03 reads as branch run 03 originating from junction box 02 on raceway path 104.\n" +
      "Branch-run numbering restarts at 01 for each junction box; each box keeps its own branch-run sequence.",
    sort_order: 12,
  },
  {
    key: "branch_origin",
    title: "Branch run origin rule",
    body:
      "A branch run MUST inherit the identifier of the junction box it physically originates from — never number " +
  ...
  },
  {
    ...
    body:
      ...
      "Branch run ^BR-\\d{3}-\\d{2}-\\d{2}$ (valid BR-104-02-01; invalid BR-104-01, BR-104-2-1).\n" +
      ...
      "(next junction box along a path, next branch run from a box) so they are not hand-constructed.\n" +
  ...
  },
  {
    ...
    body:
      ...
      "cross-checks it. Never create an artificial raceway between two boxes on the same run, and a branch run " +
...
export const STABLE_ID_REFERENCE: readonly StableIdReferenceRow[] = [
  ...
  {
    entity: "Branch Run (raceway segment)",
    format: "BR-###-##-##",
    example: "BR-104-02-03",
    notes: "Encodes raceway path + origin junction box + branch-run sequence (resets per box)",
  },
  {
