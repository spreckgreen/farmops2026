...
// ============================================================
// Move a single task from one day back to the previous day.
// Markdown ref lines, activity-log rows, and task day stamps all move
// together so the day window stays consistent.
// ============================================================

export const moveTaskToPreviousDay = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) =>
    z
      .object({
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const fromDate = data.date ?? appDateString();
    const toDate = appDateStringBefore(fromDate);
    return moveTaskBetweenDays(supabase, userId, data.taskId, fromDate, toDate);
  });

/**
 * Move a task to any farm calendar day (`toDate`), keeping note lines,
 * activity-log rows and task stamps consistent with the destination day.
 */
export const moveTaskToDay = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: unknown) =>
    z
      .object({
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const fromDate = data.date ?? appDateString();
    return moveTaskBetweenDays(supabase, userId, data.taskId, fromDate, data.toDate);
  });
