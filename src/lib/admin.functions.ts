...
import {
  ...
  type SnapshotOrigin,
} from "./snapshot-hosting";
import { assessCloudSyncedReseedReadiness, type ReseedReadiness } from "./reseed-readiness";

export type { ImportWarning, SnapshotHostingModel, SnapshotOrigin } from "./snapshot-hosting";
...
export type ImportResult = {
  ...
  target_reseed_readiness?: ReseedReadiness;
  debug?: boolean;
};
...
export type ImportApplicationDataRequest = {
  snapshot: Snapshot;
  mode: ImportMode;
  confirm?: string;
...
export async function executeImportApplicationData(params: {
  data: ImportApplicationDataRequest;
  ...
  const warnings = evaluateSnapshotImportWarnings({
    snapshotOrigin: data.snapshot.origin,
    targetOrigin: snapshotOriginFromEnv(),
    mode: data.mode,
    rewriteOwnership: data.rewriteOwnership,
  });
  const targetReseedReadiness = assessCloudSyncedReseedReadiness();
  ...
  return {
    ...
    results,
    warnings,
    target_reseed_readiness: targetReseedReadiness,
    debug: data.debug,
  };
}
...
export const importApplicationData = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<ImportResult> =>
    executeImportApplicationData({ data, context }),
  );
