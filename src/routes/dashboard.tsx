...
export const Route = createFileRoute("/dashboard")({
  ...
  head: () => ({
    meta: [
      { title: "Dashboard — Bostead Farms" },
      { name: "description", content: "Inventory dashboard for assets and stock." },
    ],
  }),
...
function DashboardPage() {
  ...
  const handleSave = async (formData: AssetFormData) => {
    const payload = {
      ...
      usage_tracking: formData.usage_tracking,
    };
    if (editingAsset) {
      ...
      <header className="border-b border-border bg-card/50 backdrop-blur-xl sticky top-0 z-50">
        <div className="container mx-auto flex items-center justify-between h-16 px-6">
          <Link to="/dashboard" className="flex items-center gap-3">
            ...
            <span className="font-heading text-xl font-bold">Inventory</span>
          </Link>
